      ...
      <main className="flex-1">
        ...
        <div className={`relative z-20 pb-20 ${currentView === 'home' && heroContent ? '-mt-16 sm:-mt-32' : 'pt-28'}`}>
          {currentView === 'home' && <>
            ...
            {!error && content.length === 0 && <div className="max-w-3xl mx-auto px-4 py-12 text-center"><div className="bg-fiat-card border border-white/10 rounded-2xl p-8 sm:p-12"><span className="text-5xl text-fiat-gold">†</span><h2 className="text-2xl sm:text-3xl font-serif font-bold mt-4">A biblioteca FIAT está sendo preparada</h2><p className="text-gray-400 mt-3">Entre com sua conta administrativa para publicar o primeiro conteúdo de formação, oração, vídeo ou áudio.</p><button onClick={() => setShowAuth(true)} className="mt-6 bg-fiat-blue border border-fiat-gold/30 px-6 py-3 rounded-xl font-bold">Entrar ou criar conta</button></div></div>}
          </>}
